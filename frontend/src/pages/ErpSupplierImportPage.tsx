...
export function ErpSupplierImportPage() {
  const { user } = useAuth();
  ...
  const uploadMutation = useMutation({
    mutationFn: () => uploadErpSuppliersImport(stationId, file!),
    onSuccess: (result) => {
      setJob(result);
      setError(null);
      setStep(3);
  ...
  return (
    <div className="rounded-lg border border-slate-200 bg-white p-6">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-semibold">Importar fornecedores ERP</h1>
          <p className="text-sm text-slate-500">Envie um arquivo CSV para sincronizar fornecedores do ERP.</p>
        </div>
        ...
        <section className="mt-6 space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">
              Colunas: erp_entity_id, erp_entity_code, erp_name, erp_cnpj
            </p>
          </div>
          <button
            type="button"
      ...
      {step === 4 && job && (
        <section className="mt-6 space-y-4">
          ...
          <Link to="/distributors" className="inline-block rounded bg-slate-900 px-4 py-2 text-sm text-white">
